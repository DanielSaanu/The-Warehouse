...
- **Decal id vs image id (black play area, no errors, decal looks fine on Creator Hub).** An Open Cloud upload
  ...
  the Output window prints `[Sprites] sheet 1: decal ... -> image ...`) and sends clients the resolved ids. If that
  line is missing or warns, fall back by hand in Studio's command bar (bottom of the Output window):
  `local d = game:GetObjects("rbxassetid://<decal id>")[1] print(d.Texture)` prints the image url; take its number
  and run `npx warehouse roblox setid 0 <image id>`, then Play. `npx warehouse roblox resolve <decal id>` shows what
  the web endpoints answer, for the curious.
